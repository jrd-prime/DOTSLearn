...
namespace GamePlay.UI.BuildingControlPanel.Part
{
    public class ProductionTimersUI
    {
        ...
        // cycle
        private VisualElement cycleCont;
        ...
        public ProductionTimersUI(VisualElement panel)
        {
            ...
            cycleCont = panel.Q<VisualElement>("cycle-cont");
            cycleLabel = cycleCont.Q<Label>();
        }

        public void UpdateTimers(int currentCycle, int maxLoads)
        {
            if (currentCycle <= maxLoads)
            {
                cycleLabel.text = $"{currentCycle} / {maxLoads}";
            }
            else
            {
                cycleLabel.text = "produced!".ToUpper();
            }

            _allTimerLabel.text = currentCycle + " sec.";
            _oneTimerLabel.text = maxLoads + " sec.";
        }
    }
}